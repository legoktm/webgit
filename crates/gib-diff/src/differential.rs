...
//!
//! One divergence is deliberate and not asserted here: a *mode-only* change
//! (say 100755 to 100644 with identical content) leaves the object IDs equal,
//! so a [`TreeDiff`] — which reports differing IDs — does not see it, while
//! `git diff-tree` calls it a modification.
...
fn mixed_repo() -> TestRepo {
    ...
    // A file replaced by a symlink: a typechange rather than an edit.
