...
impl GlobalCache {
    ...
    /// figure [`CachingRepo::about_stats`] reports.
...
/// [`CachingRepo::clear_cache`]).
async fn clear_cached_objects(db: Option<&IdbDatabase>) {
...
async fn cached_object_stats(db: Option<&IdbDatabase>) -> Option<(usize, f64)> {
    let db = db?;
    let tx = db.transaction_with_str(STORE_OBJECTS).ok()?;
    let store = tx.object_store(STORE_OBJECTS).ok()?;
